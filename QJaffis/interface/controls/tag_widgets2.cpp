...
/*****************************************************************************/
jfTagStatusPicker::jfTagStatusPicker(QString header, bool do_alternate, bool force_scrollar, QWidget* parent):QWidget(parent) {
  has_alternate = do_alternate;
  tag_data = NULL;
  internal = false;

  main_list = new QListWidget();
  if (force_scrollar) main_list->setVerticalScrollBarPolicy(Qt::ScrollBarAlwaysOn);
...
bool jfTagStatusPicker::SetListFromTags() {
  ...
  while (tagdex != (tag_data->end())) {
    ...
    if (! minsize_defined) {
      minsize = QSize(50,7 + cview->getButtonSize());
